...
// ── Enhanced Image Slider with auto-scroll + manual navigation ──
const ImageSlider = ({
  images,
  theme,
}: {
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className={`absolute inset-x-0 h-32 bg-linear-to-b from-transparent ${theme.scanLine} to-transparent pointer-events-none z-10`}
      />

      {/* Dot grid decoration */}
      <div className="absolute top-0 left-0 w-32 h-32 opacity-20 pointer-events-none z-10">
      ...
      {[...Array(8)].map((_, i) => (
        <motion.div
          ...
          className={`absolute w-1.5 h-1.5 ${theme.particle} rounded-full pointer-events-none z-10`}
          style={{ left: `${10 + Math.random() * 80}%`, top: `${10 + Math.random() * 80}%` }}
        />
      ))}

      {/* Bottom gradient overlay */}
      <div className="absolute inset-0 bg-linear-to-t from-[#0a0a0a] via-[#0a0a0a]/40 to-transparent lg:hidden pointer-events-none z-10" />
      ...
      {total > 1 && (
        <>
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.9 }}
            className="absolute left-3 top-1/2 -translate-y-1/2 z-30 w-9 h-9 rounded-full bg-black/50 backdrop-blur-md border border-white/10 flex items-center justify-center text-white hover:border-white/30 transition-all opacity-0 hover:opacity-100 group-hover:opacity-100"
          ...
          <motion.button
            ...
            whileHover={{ scale: 1.1 }}
            whileTap={{ scale: 0.9 }}
            className="absolute right-3 top-1/2 -translate-y-1/2 z-30 w-9 h-9 rounded-full bg-black/50 backdrop-blur-md border border-white/10 flex items-center justify-center text-white hover:border-white/30 transition-all opacity-0 hover:opacity-100 group-hover:opacity-100"
            style={{ opacity: undefined }}
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="absolute top-0 left-0 right-0 h-0.5 z-30 bg-white/5">
            <motion.div
              key={currentIndex}
              initial={{ width: "0%" }}
              animate={{ width: isPaused ? `${((currentIndex) / total) * 100}%` : "100%" }}
              transition={isPaused ? { duration: 0 } : { duration: 4, ease: "linear" }}
              className="h-full"
              style={{ background: `linear-gradient(to right, ${theme.primary}, ${theme.primary}80)` }}
            />
          </div>
          ...
          <div className="absolute bottom-5 left-1/2 -translate-x-1/2 flex gap-1.5 z-30">
            {images.map((_, idx) => (
              <motion.button
                key={idx}
                onClick={(e) => { e.stopPropagation(); setDragDirection(idx > currentIndex ? 1 : -1); goTo(idx); }}
                ...
                style={{ backgroundColor: idx === currentIndex ? theme.primary : 'rgba(255,255,255,0.4)' }}
              />
            ))}
          </div>

          {/* Counter badge */}
          <div className="absolute top-4 right-4 z-30 px-2.5 py-1 rounded-full bg-black/50 backdrop-blur-md border border-white/10">
          ...
          <AnimatePresence>
            {isPaused && (
              <motion.div
                ...
                className="absolute top-4 left-4 z-30 px-2.5 py-1 rounded-full bg-black/50 backdrop-blur-md border border-white/10 flex items-center gap-1.5"
              >
    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-100 flex items-center justify-center p-4 sm:p-6 lg:p-12">
          ...
          >
            <div className={`absolute top-0 left-0 w-24 h-24 border-t-2 border-l-2 ${theme.corner} rounded-tl-[2.5rem] pointer-events-none z-10`} />
            <div className={`absolute bottom-0 right-0 w-24 h-24 border-b-2 border-r-2 ${theme.corner} rounded-br-[2.5rem] pointer-events-none z-10`} />
            ...
            <div className="relative">
              ...
              <div className="flex flex-col lg:flex-row min-h-100">
                ...
                {/* ── IMAGE SLIDER ── */}
                <div className="group w-full lg:w-[60%] h-72 sm:h-96 lg:h-137.5 relative border-b lg:border-b-0 lg:border-r border-[#2a2a20] shrink-0 overflow-hidden">
                  <ImageSlider images={experience.image ?? []} theme={theme} />
                </div>
                ...
                <div className="w-full lg:w-[40%] p-6 lg:p-8 flex flex-col justify-center bg-linear-to-br from-white/5 to-transparent relative overflow-hidden">
                  ...
                  <motion.div animate={{ y: [0, -15, 0], rotate: [0, 5, 0] }} transition={{ duration: 5, repeat: Infinity, ease: "easeInOut" }} className="absolute top-6 right-6 w-16 h-16 rounded-full border border-white/5 opacity-30" />
                  <motion.div animate={{ y: [0, 20, 0], rotate: [0, -5, 0] }} transition={{ duration: 7, repeat: Infinity, ease: "easeInOut", delay: 1 }} className="absolute bottom-8 left-6 w-12 h-12 rounded-full border border-white/5 opacity-20" />

                  <motion.div initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.2 }} className="relative z-10 space-y-4">
                    
                    <div className="flex items-center gap-2">
                      <motion.div animate={{ rotate: 360 }} transition={{ duration: 20, repeat: Infinity, ease: "linear" }} className="w-6 h-6 rounded-full border-2 border-dashed opacity-30" style={{ borderColor: theme.primary }} />
                      <motion.span whileHover={{ scale: 1.05, x: 4 }} className={`relative inline-block px-4 py-2 rounded-full bg-linear-to-r ${theme.badge} text-[10px] font-black tracking-[0.2em] uppercase overflow-hidden group cursor-default shadow-lg`}>
                    ...
                    <div className="space-y-2">
                      <motion.div initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ delay: 0.4, duration: 0.8 }} className="h-0.5 w-12 rounded-full origin-left" style={{ background: `linear-gradient(to right, ${theme.primary}, transparent)` }} />
                      <motion.h2 whileHover={{ x: 4 }} className="text-xl sm:text-2xl lg:text-3xl font-black text-white uppercase tracking-tighter leading-tight relative" style={{ textShadow: `0 0 12px ${theme.glow}` }}>
                        {experience.title}
                        <motion.div initial={{ width: 0 }} animate={{ width: "100%" }} transition={{ delay: 0.6, duration: 0.8 }} className="absolute -bottom-0.5 left-0 h-px bg-linear-to-r from-white/20 to-transparent" />
                      </motion.h2>
                    </div>

                    <motion.div whileHover={{ x: 4 }} className="flex items-center gap-2.5 py-3 px-3 bg-white/5 rounded-lg border border-white/10 backdrop-blur-sm">
                      ...
                      <div className="flex-1">
                        ...
                        <p className={`text-transparent bg-clip-text bg-linear-to-r ${theme.text} text-sm sm:text-base font-bold tracking-tight`}>{experience.role}</p>
                      </div>
                    </motion.div>
                    
                    <motion.div whileHover={{ x: 4 }} className="flex items-center gap-2.5 pb-4 border-b relative" style={{ borderColor: `${theme.primary}20` }}>
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs sm:text-sm text-white font-semibold">{experience.company}</p>
                      </div>
                      <motion.div animate={{ scale: [1, 1.2, 1] }} transition={{ duration: 2, repeat: Infinity }} className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: theme.primary, boxShadow: `0 0 10px ${theme.glow}` }} />
                    </motion.div>
                    ...
                    <div className="flex gap-3 pt-4 relative">
                      ...
                      <motion.div whileHover={{ y: -4, scale: 1.02 }} className="flex-1 group cursor-default">
                        <div className="p-3 rounded-lg bg-linear-to-br from-white/10 to-white/5 border border-white/10 backdrop-blur-sm space-y-2">
                          ...
                          <div className="text-lg sm:text-xl font-black bg-linear-to-br from-white to-gray-400 bg-clip-text text-transparent">{experience.mode || "Work"}</div>
                        </div>
                      </motion.div>
                      ...
                      <motion.div whileHover={{ y: -4, scale: 1.02 }} className="flex-1 group cursor-default">
                        <div className="p-3 rounded-lg bg-linear-to-br from-white/10 to-white/5 border border-white/10 backdrop-blur-sm space-y-2 relative overflow-hidden">
                          <motion.div animate={{ opacity: [0.2, 0.5, 0.2] }} transition={{ duration: 2, repeat: Infinity }} className="absolute inset-0 blur-xl" style={{ background: `radial-gradient(circle at center, ${theme.primary}40, transparent)` }} />
                          <div className="relative flex items-center gap-1.5">
                            <motion.div animate={{ rotate: 360 }} transition={{ duration: 3, repeat: Infinity, ease: "linear" }}>
                              <svg className="w-3.5 h-3.5" style={{ color: theme.primary }} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
                            </motion.div>
                            <div className="text-[8px] uppercase tracking-[0.2em] text-gray-500 font-bold">Priority</div>
                          </div>
                          <div className="relative text-lg sm:text-xl font-black" style={{ color: theme.primary, textShadow: `0 0 10px ${theme.glow}` }}>{experience.priority || "High"}</div>
                        </div>
                      </motion.div>
                    </div>
                ...
                <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.4 }} className="space-y-6">
                  ...
                  <div className="flex flex-col gap-4">
                    {experience.links && experience.links.map((link, idx) => (
                      <motion.a key={idx} href={link} target="_blank" rel="noopener noreferrer" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="relative flex items-center justify-between w-full px-6 py-5 bg-white text-black rounded-2xl font-black text-xs uppercase tracking-widest group overflow-hidden shadow-lg">
                        ...
                        <span className="relative z-10 group-hover:text-white transition-colors">{link.includes('facebook') ? 'Watch on Facebook' : link.includes('youtube') ? 'Watch on YouTube' : `View Link ${idx + 1}`}</span>
                        <motion.svg className="w-5 h-5 relative z-10 group-hover:text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></motion.svg>
                      </motion.a>
                    ))}
                  </div>
                  ...
                  <div className="pt-8 space-y-6 p-6 bg-linear-to-br from-white/5 to-transparent border border-white/10 rounded-2xl">
                    ...
                    <div className="flex items-center justify-between group"><span className="text-gray-500 text-[10px] font-bold uppercase tracking-widest flex items-center gap-2"><div className="w-1.5 h-1.5 rounded-full bg-gray-500" />Location</span><span className={`text-white text-xs font-bold ${theme.hoverText} transition-colors`}>{experience.location}</span></div>
                  </div>
                </motion.div>
              </div>
            </div>
